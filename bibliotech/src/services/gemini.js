...
export async function getBookRecommendations(book) {
  if (!genAI) {
    return [
      { titulo: 'El Hobbit', razon: 'Comparte el universo de fantasía', relevancia: 0.9 },
      { titulo: 'Las Crónicas de Narnia', razon: 'Aventura fantástica similar', relevancia: 0.8 },
    ]
  }
...
export async function chatWithGemini(messages) {
  ...
  const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' })
  const systemPrompt = 'Eres un asistente de biblioteca amigable. Ayudas a los usuarios a encontrar libros, responder preguntas literarias y recomendar lecturas. Sé conciso y útil.'
